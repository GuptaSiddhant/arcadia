...
def pay_checkout_view(request, game_id):
    ...
    # Do not know PID yet
    pid = 123
    checksumstr = "pid={}&sid={}&amount={}&token={}".format(pid, settings.sid, game.price, settings.SECRET_KEY)
    checksum = md5(checksumstr.encode("ascii")).hexdigest()

    args = {'game': game, 'redirect': red_tag, 'checksum': checksum, 'sid': settings.sid, 'pid': pid}
...
def payment_result_view(request):
    ...
    checksumstr = "pid={}&ref={}&result={}&token={}".format(pid_tag, reference_tag, result_tag, settings.SECRET_KEY)
    checksum = md5(checksumstr.encode("ascii")).hexdigest()
